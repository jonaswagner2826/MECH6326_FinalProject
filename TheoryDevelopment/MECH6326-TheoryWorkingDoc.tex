...
\] and the associated success/fail noise $\forall_{i=pc,mn}$ be derived as \[
    w_{i,sf} = \begin{cases}
        0 & ({\color{red} 5} + w_{i,d20} \leq {\color{red} 15} \text{ OR } w_{i,d20} = 1) \text{ AND } w_{i,d20} \neq 1\\
        1 & ({\color{red} 5} + w_{i,d20} > {\color{red} 15} \text{ OR } w_{i,d20} = 20) \text{ AND } w_{i,d20} \neq 1\\
    \end{cases}
...
\begin{itemize}
    ...
    \item The players action:\[
        f_{pc,a} = \begin{cases}
            ...
            \mqty[0 & -w_{pc,sf}({\color{red}0} + w_{pc,d6}) & 0]^T & u_{pc,a} = \text{Melee} \text{ AND } \norm{x_{pc,p} - x_{mn,p}}_1 \leq \text{Melee Range}\\
            \mqty[0 & -w_{pc,sf}({\color{red}0} + w_{pc,d8}) & 0]^T & u_{pc,a} = \text{Ranged} \text{ AND } \text{Melee Range} < \norm{x_{pc,p} - x_{mn,p}}_1 \leq \text{Ranged Range}\\
    ...
    \item The monsters state dependent action: \[
        f_{mn,a} = \begin{cases}
            \mqty[-w_{mn,sf}({\color{red}0} + w_{mn,d6}) & 0 & 0]^T & \norm{x_{pc,p} - x_{mn,p}}_1 \leq \text{Melee Range}\\
            \mqty[- w_{mn,sf}({\color{red}0} + w_{mn,d8}) & 0 & 0]^T & \text{Melee Range} < \norm{x_{pc,p} - x_{mn,p}}_1 \leq \text{Ranged Range}\\
