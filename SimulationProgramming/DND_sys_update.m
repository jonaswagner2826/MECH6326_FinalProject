function [x_new, pc_sf, mn_sf] = DND_sys_update(x,u,w,const)
    ...
    % Monster Dies
    x_new.mn.x = max(min(x_new.mn.x,x_new.pc.x + const.relPosMax),x_new.pc.x - const.relPosMax);
    x_new.mn.y = max(min(x_new.mn.y,x_new.pc.y + const.relPosMax),x_new.pc.y - const.relPosMax);
